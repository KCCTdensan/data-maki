...
+ 最終盤面を下の行から順に走査し、各行の0-3の各要素数を揃える
  ...
  + 残った過剰な要素のピースを、そのピース近隣列の不足した要素で置換する
+ 最終盤面を右列から順に走査し、各列を横方向の寄せのみで揃える

== ピースの移動法

ピースの移動は、目標位置までに存在するピースを抜いて移動させる。定型抜き型のみの使用では、移動させたい距離を$N$と置くと必要手数は$O(log N)$手となる。
...
また、一方向に送受信可能なデータチャネルを用いて相互に通信を行えるようにしている。

実行時は、最適化された単独バイナリとして実行できるようにビルドする。

== UI

Reactを利用し、サーバからのイベントストリームを受け取り、ソルバの進捗をリアルタイムで表示する。
ソルバと完全に分離することで、長時間の計算でもUIがブロックされない。
