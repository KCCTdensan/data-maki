...
前述の流れは、下の行から右の列という順番になっている。 しかし、走査の始点と終点で計8通りのパターンがあるため、全て検証し、最適な流れを見つける。
...
Promiseを利用した非同期処理をベースに、並列化のため、アルゴリズム部をWebWorkerに分割してメインスレッドから分離している。
...
Reactを利用し、サーバからのイベントストリームを受け取り、ソルバの進捗をリアルタイムで表示するようにしている。
ソルバと完全に分離することで、時間のかかる処理が行われている間もUIはブロックされない。
