...
+ 最終盤面を下の行から順に見ていき、各行の0-3の各要素の数を揃える
  ...
  + その後、残った過剰な要素のピースを、そのピースに近 い列にある不足した要素を置き換えていく
+ 最終盤面を右の列から順に見ていき、各列を横方向の寄せのみで揃える

== ピースの移動法

ピースの移動については、単純に移動させたい位置までに存在するピースを抜いて移動させようと思うと、定型抜き型のみの使用では、移動させたい距離をNと置くと必要手数はO(logN)手となる。しかし、盤面の端に移動させるだけの場合は、移動させたいピースを抜く方法を取ることで、1手で移動させることができる。これを利用して、各行・列を揃える際に一番下の行・一番右の列に揃える。そして、揃えた行・列を反対側に移動させていくことで、少ない手数で順番に盤面を揃えていくことができるという算段だ。

また、(1.2)では、揃えている行と平行に移動させる必要がある。ここでは盤面の端に揃えるということは難しいので、先ほど述べたO(logN)手で揃えることになる。しかし、揃えている途中である一番下の行があるため、そこを回避して型抜きをしていく必要がある。そこで、定型抜き型のタイプⅡを使用することで、盤面が崩れてしまうことを防ぐ。

== 複数パターンの流れ

前述した流れは、下の行から右の列という順番の流れになっている。しかし、最初にどの方向から始めるかにより4通り、その後どちらの横方向から揃えていくかにより2通りの、計8通りの分岐がある。もちろんそれぞれのパターンで必要な手数は異なってくるので、それぞれのパターンを検証することにより最適な流れを見つけることができる。これを実現するために、初めから盤面をそれぞれ反転させた状態で同じような流れを並列処理で行う。
